...
# It originally was going to be a daemon, constantly running in the background making new word images for the data_sets to read in, but I realized the generation was fast enough that having the data_sets multithreaded would probably be good enough.
# I only used in as an object to be called. The daemon functionality is not fully implemented (and none of my data_sets have the interface to use the daemon data)

# Default arguments for non-daemon functionality
class RenderText: # FORMERLY GenDaemon
    ...
    def generate(self, text=None, font=None, ret_font=False, used=None):
        ...
        if font is None:
            # fontR,filename,[fontNums,minY,maxY,hasLower,hasBrackets],filenameNums = self.gen.getFont()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='run GenDaemon to continuously generate word images from wikipedia (en) text')
    ...
    daemon = RenderText(args.font_dir, args.out_dir, args.num_held)
