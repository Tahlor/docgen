...
class Cache:
    def __init__(self, capacity=300, replace_after=10, min_count_before_random=10):
    ...
    def get(self, key, random_ok=True):
        if key not in self.cache and random_ok and ( len(self.cache) >= self.capacity or
                                                     (len(self) > self.min_count_before_random and random.random() < 0.5)):
