...
def process_args(args):
    ...
    if args.output_ocr_json is None:
        args.output_ocr_json = args.output_folder / "OCR.json"
    else:
        args.output_ocr_json = Path(args.output_ocr_json)
    if args.output_coco_json is None:
        args.output_coco_json = args.output_folder / "COCO.json"
    else:
        args.output_coco_json = Path(args.output_coco_json)
...
def main(args=None):
    ...
    BOX_FILLER = BoxFiller(default_max_lines=args.max_lines,
    ...
    try:
        ...
        logger.exception(e)
        warnings.warn(f"Only generated {IDX} out of {args.count} images")
    coco = ocr_dataset_to_coco(ocr_dict, "French Lines - v0.0.1.0 Alpha", exclude_cats="word")
    with args.output_coco_json.open("w") as f:
        json.dump(coco, f)
...
def process_batch(d, renderer, args):
    ...
    """
    global ocr_dict, IDX, OUTPUT_PATH
